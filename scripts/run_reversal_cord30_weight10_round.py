...
"""
Run the single-candidate lightweight cord30 partner follow-up round.
"""
...
ROUND_ID = "rr_exploratory_reversal_cord30_weight10_20260506"
...
SECONDARY_REFERENCE_RUN_ID = "confirmatory_reversal_p98_cord30_trainval_20260506"
CHALLENGER_CANDIDATE_ID = "reversal_p98_cord30_w10_v1"
CHALLENGER_RUN_ID = "exploratory_reversal_p98_cord30_w10_trainval_20260506"

CODE_HASH_FILES = [
    SCRIPTS_DIR / "run_reversal_cord30_weight10_round.py",
...
def main() -> None:
    ...
    payload = {
        ...
        "round_decision": "WEIGHT10_PASS" if evaluation["all_pass"] else "WEIGHT10_FAIL",
    }
    write_json(ROUND_DIR / "weight10_results.json", payload)

    lines = [
        "# Reversal + Cord30 Weight-10 Results",
        "",
    ...
    (ROUND_DIR / "weight10_results.md").write_text("\n".join(lines).rstrip() + "\n", encoding="utf-8")

    timestamp = now_iso()
    append_jsonl(
        REGISTRY_DIR / "research_round_registry.jsonl",
        {
            "registered_at": "2026-05-06T12:40:00+08:00",
            "research_round_id": ROUND_ID,
            "research_tier": "exploratory",
            "status": "completed_weight10_pass" if evaluation["all_pass"] else "completed_weight10_fail",
            "round_type": "weighted_partner_followup",
            ...
            "notes": "Single-candidate lightweight cord30 follow-up after weight screen selected 10% partner weight."
        }
    )
    append_jsonl(
        REGISTRY_DIR / "candidate_scheme_registry.jsonl",
        {
            "registered_at": "2026-05-06T12:40:00+08:00",
            "candidate_scheme_id": CHALLENGER_CANDIDATE_ID,
            "scheme_family": "reversal_tail_handled_composite_weighted",
            "status": "weight10_passed" if evaluation["all_pass"] else "weight10_failed",
            "research_round_id": ROUND_ID,
            "research_tier": "exploratory",
            "owner": "codex",
            ...
            "score_rule": "0.9 * percent_rank(p98_reversal_score) + 0.1 * percent_rank(cord30_score)",
            "baseline_reference_candidate_scheme_id": "reversal_tail_exclude_p98_v1",
            "snapshot_id": load_json(CONTRACT_PATH)["snapshot_id"],
            ...
            "status_updated_at": timestamp,
            "notes": "Lightweight cord30 follow-up chosen from the cheap weight screen."
        }
    )
