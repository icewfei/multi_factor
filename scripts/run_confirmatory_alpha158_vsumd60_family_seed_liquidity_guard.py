...
"""
Run the first restrained VSUMD60 family-seed confirmatory round.

The score stays fixed on standalone Alpha158 VSUMD60. This seed changes exactly one
core dimension: add a ranking liquidity guard at the run-state stage.
"""
...
ROUND_ID = "rr_confirmatory_alpha158_vsumd60_family_seed_liquidity_guard_20260429"
...
CHALLENGER_CANDIDATE_ID = "confirmatory_alpha158_vsumd60_seed_liquidity_guard70_v1"
CHALLENGER_RUN_ID = "confirmatory_vsumd60_seed_liquidity_guard70_trainval_20260429"
CHALLENGER_FIXED_TEST_RUN_ID = "confirmatory_fixed_test_vsumd60_seed_liquidity_guard70_trainval_20260429"
FEATURE_NAME = "VSUMD60"
VALIDATION_START = "20190101"
VALIDATION_END = "20211231"

CODE_HASH_FILES = [
    SCRIPTS_DIR / "run_confirmatory_alpha158_vsumd60_family_seed_liquidity_guard.py",
...
def main() -> None:
    ...
    pass_boolean = bool(
        validation_annual_relative_return_delta_vs_vsumd60 >= -0.02
        and validation_relative_ir_delta_vs_vsumd60 >= -0.10
        and validation_max_drawdown_delta_vs_vsumd60 >= 0.00
    ...
    summary = {
        ...
        "decision_rule": "(validation_annual_relative_return_delta_vs_vsumd60 >= -0.02) AND (validation_relative_ir_delta_vs_vsumd60 >= -0.10) AND (validation_max_drawdown_delta_vs_vsumd60 >= 0.00) AND (validation_avg_turnover_daily_delta_vs_vsumd60 <= -0.01) AND (validation_low_liquidity_weight_share_delta_vs_vsumd60 <= 0.00) AND (validation_cost_stress_annual_relative_return_delta_vs_vsumd60 >= 0.00)",
        "pass_boolean": pass_boolean,
        "round_decision": "KEEP" if pass_boolean else "REJECT",
        "conclusion": (
            "The first VSUMD60 family seed improved tradability and cost robustness without giving back too much standalone edge."
            if pass_boolean
            else "The first VSUMD60 family seed failed to improve tradability enough to justify giving up standalone edge."
        ),
    }
