...
"""
Run the single-candidate liquidity-guard deployment-fix round for the
reversal+cord30 composite.
"""
...
ROUND_ID = "rr_exploratory_reversal_cord30_liquidity_guard60_20260506"
...
UNGUARDED_REFERENCE_RUN_ID = "confirmatory_reversal_p98_cord30_trainval_20260506"
CHALLENGER_CANDIDATE_ID = "reversal_p98_cord30_liqguard60_v1"
CHALLENGER_RUN_ID = "exploratory_reversal_p98_cord30_liqguard60_trainval_20260506"

CODE_HASH_FILES = [
    SCRIPTS_DIR / "run_reversal_cord30_liquidity_guard60_round.py",
...
def main() -> None:
    ...
    unguarded_reference = load_json(FIXED_TEST_DIR / UNGUARDED_REFERENCE_RUN_ID / "validation_readout.json")
    ...
    payload = {
        ...
        "frozen_baseline": {
            "run_id": BASELINE_FIXED_RUN_ID,
            "validation": baseline_validation,
            "cost_stress": baseline_cost,
            "low_liquidity": baseline_liq,
        },
        "secondary_reference": {
            "run_id": UNGUARDED_REFERENCE_RUN_ID,
            "validation": unguarded_reference,
        },
        "challenger": {
        ...
        "round_decision": "GUARD60_PASS" if evaluation["all_pass"] else "GUARD60_FAIL"
    }
    write_json(ROUND_DIR / "guard60_results.json", payload)

    lines = [
        "# Reversal + Cord30 Liquidity Guard 0.60 Results",
        "",
    ...
    (ROUND_DIR / "guard60_results.md").write_text("\n".join(lines).rstrip() + "\n", encoding="utf-8")

    timestamp = now_iso()
    append_jsonl(
        REGISTRY_DIR / "research_round_registry.jsonl",
        {
            "registered_at": "2026-05-06T12:12:00+08:00",
            "research_round_id": ROUND_ID,
            "research_tier": "exploratory",
            "status": "completed_guard60_pass" if evaluation["all_pass"] else "completed_guard60_fail",
            "round_type": "deployment_fix",
            "research_question": load_json(ROUND_DIR / "preregistration.json")["research_question"],
            "allowed_core_dimension": "ranking eligibility guard only",
            "changed_dimension": "ranking_eligibility_guard",
            ...
            "notes": "Single threshold liquidity-guard round completed after deployment-loss diagnosis and cheap guard screen."
        }
    )
    append_jsonl(
        REGISTRY_DIR / "candidate_scheme_registry.jsonl",
        {
            "registered_at": "2026-05-06T12:12:00+08:00",
            "candidate_scheme_id": CHALLENGER_CANDIDATE_ID,
            "scheme_family": "reversal_tail_handled_composite_guarded",
            "status": "guard60_passed" if evaluation["all_pass"] else "guard60_failed",
            "research_round_id": ROUND_ID,
            "research_tier": "exploratory",
            "owner": "codex",
            ...
            "status_updated_at": timestamp,
            "notes": "Liquidity-guard deployment-fix candidate after the unguarded composite failed promotion."
        }
    )
