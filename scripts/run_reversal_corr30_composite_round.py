...
"""
Run the single-candidate p98 reversal + corr30 composite round.
"""
...
ROUND_ID = "rr_exploratory_reversal_corr30_composite_20260506"
...
CHALLENGER_CANDIDATE_ID = "reversal_p98_corr30_ew_v1"
CHALLENGER_RUN_ID = "exploratory_reversal_p98_corr30_trainval_20260506"

CODE_HASH_FILES = [
    SCRIPTS_DIR / "run_reversal_corr30_composite_round.py",
...
def main() -> None:
    ...
    validation_annual_relative_return_delta = challenger_validation["annual_relative_return"] - baseline_validation["annual_relative_return"]
    ...
    validation_avg_turnover_daily_delta = challenger_validation["avg_turnover_daily"] - baseline_validation["avg_turnover_daily"]
    cost_stress_annual_relative_return_delta = challenger_cost["annual_relative_return"] - baseline_cost["annual_relative_return"]
    low_liquidity_weight_share_delta = challenger_liq["low_liquidity_weight_share"] - baseline_liq["low_liquidity_weight_share"]
    ...
    payload = {
        ...
        "round_decision": "CORR30_PASS" if evaluation["all_pass"] else "CORR30_FAIL"
    }
    write_json(ROUND_DIR / "corr30_results.json", payload)

    lines = [
        "# Reversal + CORR30 Results",
        "",
    ...
    (ROUND_DIR / "corr30_results.md").write_text("\n".join(lines).rstrip() + "\n", encoding="utf-8")

    timestamp = now_iso()
    append_jsonl(
        REGISTRY_DIR / "research_round_registry.jsonl",
        {
            "registered_at": "2026-05-06T12:24:00+08:00",
            "research_round_id": ROUND_ID,
            "research_tier": "exploratory",
            "status": "completed_corr30_pass" if evaluation["all_pass"] else "completed_corr30_fail",
            "round_type": "minimal_composite_check",
            ...
            "notes": "Single-candidate corr30 partner round completed after the cord30 partner line failed."
        }
    )
    append_jsonl(
        REGISTRY_DIR / "candidate_scheme_registry.jsonl",
        {
            ...
            "status": "corr30_passed" if evaluation["all_pass"] else "corr30_failed",
            "research_round_id": ROUND_ID,
            "research_tier": "exploratory",
            "owner": "codex",
            "score_builder": "build_reversal_tail_composite_model_scores.py",
            "feature_source": "reversal_run_state + corr30_run_state",
            "feature_set": ["negated_reversal_p98_tail_handled", "alpha158_corr30"],
            "score_rule": "0.5 * percent_rank(p98_reversal_score) + 0.5 * percent_rank(corr30_score)",
            "baseline_reference_candidate_scheme_id": "reversal_tail_exclude_p98_v1",
            "snapshot_id": load_json(CONTRACT_PATH)["snapshot_id"],
            ...
            "status_updated_at": timestamp,
            "notes": "p98 + corr30 exploratory partner check after cord30 line closure."
        }
    )
