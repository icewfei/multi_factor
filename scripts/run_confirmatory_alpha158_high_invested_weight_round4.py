...
"""
Run the fourth confirmatory Alpha158 standalone shortlist round on the trainval-only
snapshot and project panels, focused on higher-invested-weight head-extraction
reserve cards.
"""
...
ROUND_ID = "rr_confirmatory_alpha158_high_invested_weight_head_extraction_shortlist_round4_20260430"
...
SHORTLIST = [
    {
        ...
        "feature_name": "IMXD5",
        "mechanism": "path_ordering_breakout_timing",
        "run_id": "confirmatory_imxd5_trainval_20260430_round4",
        "fixed_test_run_id": "confirmatory_fixed_test_imxd5_trainval_20260430_round4",
    },
    {
        "candidate_scheme_id": "price_volume_single_signal_alpha158_cord30_v1",
        "feature_name": "CORD30",
        "mechanism": "price_volume_change_correlation_head_extraction",
        "run_id": "confirmatory_cord30_trainval_20260430_round4",
        "fixed_test_run_id": "confirmatory_fixed_test_cord30_trainval_20260430_round4",
    },
    {
        "candidate_scheme_id": "price_volume_single_signal_alpha158_imax20_v1",
        "feature_name": "IMAX20",
        "mechanism": "breakout_freshness_head_extraction",
        "run_id": "confirmatory_imax20_trainval_20260430_round4",
        "fixed_test_run_id": "confirmatory_fixed_test_imax20_trainval_20260430_round4",
    },
]
CODE_HASH_FILES = [
    SCRIPTS_DIR / "run_confirmatory_alpha158_high_invested_weight_round4.py",
...
def run_candidate(candidate: dict) -> dict:
    ...
    fixed_test_run_id = candidate["fixed_test_run_id"]
...
def decide(reference: dict, payload: dict) -> dict:
    ...
    pass_boolean = bool(
        ...
        and avg_turnover_daily_delta <= 0.04
        and payload.get("avg_invested_weight", 0.0) >= 0.18
    )
    return {
        ...
        "boolean_expression": (
            ...
            "AND (validation_avg_turnover_daily - reference_validation_avg_turnover_daily <= 0.04) "
            "AND (validation_avg_invested_weight >= 0.18)"
        ),
    }
...
def main() -> None:
    ...
    write_json(ROUND_DIR / "confirmatory_round4_results.json", summary)
    ...
    for row in top_ranked:
        lines.extend(
            [
                ...
                f"  - `validation_avg_invested_weight(验证期平均投资仓位) = {row['avg_invested_weight']:.6f}`" if row.get("avg_invested_weight") is not None else "  - `validation_avg_invested_weight(验证期平均投资仓位) = null`",
    ...
    (ROUND_DIR / "confirmatory_round4_results.md").write_text("\n".join(lines).rstrip() + "\n", encoding="utf-8")
