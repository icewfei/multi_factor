...
ROUND_ID = "rr_price_volume_volume_price_synchronicity_composability_screen_20260425"
CANDIDATE_ID = "price_volume_screen_volume_price_synchronicity_20d_composability_v1"
...
RUN_ID = "screendiag_price_volume_volume_price_synchronicity_composability_20260425"
...
def main() -> None:
    ...
    single_signal_report = load_json(
        RESEARCH_DIR
        / "research_rounds"
        / "rr_price_volume_single_signal_discovery_v18_round5_20260425"
        / "price_volume_single_signal_volume_price_synchronicity_20d_v1_signal_edge_diagnosis_20260425.json"
    )

    round_dir = RESEARCH_DIR / "research_rounds" / ROUND_ID
    json_report_path = round_dir / "price_volume_screen_volume_price_synchronicity_20d_composability_v1_diagnosis_20260425.json"
    md_report_path = round_dir / "price_volume_screen_volume_price_synchronicity_20d_composability_v1_diagnosis_20260425.md"
    ...
    try:
        ...
        con.execute(
            f"""
            CREATE OR REPLACE VIEW volume_sync_features AS
            WITH bars AS (
                SELECT
        ...
        con.execute(
            ...
            SELECT
                ...
                r.liquidity_rank,
                s.volume_price_synchronicity_20d_raw
            ...
            LEFT JOIN volume_sync_features s
        ...
        con.execute(
            ...
            SELECT
                ...
                PERCENT_RANK() OVER (
                    ...
                    ORDER BY volume_price_synchronicity_20d_raw DESC, instrument ASC
                ) AS volume_price_synchronicity_rank
            FROM base_frame
            WHERE ranking_eligible_D0
              AND volume_price_synchronicity_20d_raw IS NOT NULL
            """
        )
        con.execute(
            """
            ...
            SELECT
                ...
                b.liquidity_rank,
                s.volume_price_synchronicity_rank,
                CASE
                ...
                CASE
                    WHEN b.momentum_rank IS NOT NULL
                     ...
                     AND s.volume_price_synchronicity_rank IS NOT NULL
                    THEN (b.momentum_rank + b.liquidity_trend_rank + s.volume_price_synchronicity_rank) / 3.0
                    ELSE NULL
        ...
        def bucket_readout() -> dict:
            rows = con.execute(
                ...
                bucketed AS (
                    SELECT
                        ...
                        NTILE(3) OVER (
                            PARTITION BY signal_date
                            ORDER BY volume_price_synchronicity_rank DESC, instrument ASC
                        ) AS signal_bucket
                    FROM usable
                )
                SELECT
            ...
            for core_bucket, signal_bucket, avg_label in rows:
                mapping[f"core_{labels[core_bucket]}_sync_{labels[signal_bucket]}"] = avg_label
            return mapping
        ...
        momentum_signal_corr = pair_corr("momentum_rank", "volume_price_synchronicity_rank")
        liquidity_signal_corr = pair_corr("liquidity_trend_rank", "volume_price_synchronicity_rank")
        core_signal_corr = pair_corr("core_score_v18", "volume_price_synchronicity_rank")
    ...
    audit = {
        ...
        "notes": [
            ...
            "The screen score is a tentative static three-component average over momentum_60_5_raw, liquidity_trend_20_60_raw, and volume_price_synchronicity_20d_raw.",
    ...
    additive_high_bucket = (
        bucket_stats.get("core_high_sync_high") is not None
        and bucket_stats.get("core_high_sync_mid") is not None
        and bucket_stats["core_high_sync_high"] >= bucket_stats["core_high_sync_mid"]
    )
    ...
    if top_slice_improved and head_stability_not_worse and liquidity_not_worse and additive_high_bucket:
        ...
        judgement["primary_conclusion"] = (
            "volume_price_synchronicity_20d_raw appears compositionally promising relative to the frozen v18 core family: "
        ...
        judgement["implication_for_next_step"] = (
            "A future family round may test volume_price_synchronicity_20d_raw as a constrained third signal under a single-dimension preregistration."
        )
    else:
        judgement["classification"] = "composability_screen_mixed"
        judgement["primary_conclusion"] = (
            "volume_price_synchronicity_20d_raw is a real standalone keeper, but its additive fit with the frozen v18 core family is mixed rather than clean. "
        ...
        judgement["implication_for_next_step"] = (
            "Do not promote volume_price_synchronicity_20d_raw into a direct equal-weight family replacement yet. If used later, prefer a constrained third-signal test or gated modifier design."
        )

    if momentum_signal_corr["corr_avg_daily"] is not None:
        judgement["mechanism"].append(
            "Pairwise momentum-vs-volume-synchronicity rank correlation is "
    ...
    if core_signal_corr["corr_avg_daily"] is not None:
        judgement["mechanism"].append(
            "Core-family-vs-volume-synchronicity rank correlation is "
    ...
    if bucket_stats.get("core_high_sync_high") is not None and bucket_stats.get("core_high_sync_mid") is not None:
        judgement["mechanism"].append(
            "In the high-core region, core_high+sync_high average label is "
            + format_num(bucket_stats["core_high_sync_high"])
            + " versus core_high+sync_mid "
            + format_num(bucket_stats["core_high_sync_mid"])
    ...
    report = {
        ...
        "research_round_id": ROUND_ID,
        "as_of_date": "20260425",
        "diagnosis_question": "Relative to the frozen v18 core two-signal family, does volume_price_synchronicity_20d_raw look compositionally compatible enough to justify a later family-level promotion?",
        ...
        "pairwise_component_correlation": {
            "momentum_vs_volume_price_synchronicity": momentum_signal_corr,
            "liquidity_trend_vs_volume_price_synchronicity": liquidity_signal_corr,
            "core_v18_vs_volume_price_synchronicity": core_signal_corr
        },
    ...
    md_lines = [
        ...
        "",
        f"- `momentum_vs_volume_price_synchronicity_corr_avg_daily(动量与量价同步日均相关) = {format_num(momentum_signal_corr['corr_avg_daily'])}`",
        f"- `liquidity_trend_vs_volume_price_synchronicity_corr_avg_daily(流动性趋势与量价同步日均相关) = {format_num(liquidity_signal_corr['corr_avg_daily'])}`",
        f"- `core_v18_vs_volume_price_synchronicity_corr_avg_daily(v18核心分数与量价同步日均相关) = {format_num(core_signal_corr['corr_avg_daily'])}`",
        f"- `core_high_sync_high_avg_label(核心高+量价同步高平均标签) = {format_num(bucket_stats.get('core_high_sync_high'))}`",
        f"- `core_high_sync_mid_avg_label(核心高+量价同步中平均标签) = {format_num(bucket_stats.get('core_high_sync_mid'))}`",
        f"- `core_high_sync_low_avg_label(核心高+量价同步低平均标签) = {format_num(bucket_stats.get('core_high_sync_low'))}`",
        "",
