...
ROUND_ID = "rr_price_volume_amount_shock_composability_screen_20260424"
CANDIDATE_ID = "price_volume_screen_amount_shock_5_20_composability_v1"
...
RUN_ID = "screendiag_price_volume_amount_shock_composability_20260424"
...
def main() -> None:
    ...
    single_signal_report = load_json(
        RESEARCH_DIR
        / "research_rounds"
        / "rr_price_volume_single_signal_discovery_v18_round4_20260423"
        / "price_volume_single_signal_amount_shock_5_20_v1_signal_edge_diagnosis_20260423.json"
    )

    round_dir = RESEARCH_DIR / "research_rounds" / ROUND_ID
    json_report_path = round_dir / "price_volume_screen_amount_shock_5_20_composability_v1_diagnosis_20260424.json"
    md_report_path = round_dir / "price_volume_screen_amount_shock_5_20_composability_v1_diagnosis_20260424.md"
    ...
    try:
        ...
        con.execute(
            f"""
            CREATE OR REPLACE VIEW amount_features AS
            WITH bars AS (
                SELECT
        ...
        con.execute(
            ...
            LEFT JOIN amount_features a
              ON p.instrument = a.instrument
             AND p.signal_date = a.signal_date
            """
        )
        con.execute(
            """
            CREATE OR REPLACE VIEW amount_ranks AS
            SELECT
                snapshot_id,
                instrument,
                signal_date,
            ...
            SELECT
                ...
                CASE
                    WHEN b.momentum_rank IS NOT NULL
                     ...
                     AND a.amount_shock_rank IS NOT NULL
                    THEN (b.momentum_rank + b.liquidity_trend_rank + a.amount_shock_rank) / 3.0
                    ELSE NULL
                END AS screen_model_score_D0
            FROM base_frame b
            LEFT JOIN amount_ranks a
              ON b.snapshot_id = a.snapshot_id
             AND b.instrument = a.instrument
             AND b.signal_date = a.signal_date
            """
        )
        con.execute(
            f"""
            ...
            rows = con.execute(
                ...
                bucketed AS (
                    SELECT
                        ...
                        NTILE(3) OVER (
                            PARTITION BY signal_date
                            ORDER BY amount_shock_rank DESC, instrument ASC
                        ) AS amount_bucket
                    FROM usable
                )
                SELECT
                ...
                FROM bucketed
                GROUP BY core_bucket, amount_bucket
                ORDER BY core_bucket, amount_bucket
                """
            ).fetchall()
            mapping = {}
            labels = {1: "high", 2: "mid", 3: "low"}
            for core_bucket, amount_bucket, avg_label in rows:
                mapping[f"core_{labels[core_bucket]}_amount_{labels[amount_bucket]}"] = avg_label
            return mapping
        ...
        momentum_amount_corr = pair_corr("momentum_rank", "amount_shock_rank")
        liquidity_amount_corr = pair_corr("liquidity_trend_rank", "amount_shock_rank")
        core_amount_corr = pair_corr("core_score_v18", "amount_shock_rank")
    ...
    audit = {
        ...
        "notes": [
            ...
            "The screen score is a tentative static three-component average over momentum_60_5_raw, liquidity_trend_20_60_raw, and amount_shock_5_20_raw.",
    ...
    additive_high_bucket = (
        bucket_stats.get("core_high_amount_high") is not None
        and bucket_stats.get("core_high_amount_mid") is not None
        and bucket_stats["core_high_amount_high"] >= bucket_stats["core_high_amount_mid"]
    )
    ...
    if top_slice_improved and head_stability_not_worse and liquidity_not_worse and additive_high_bucket:
        ...
        judgement["primary_conclusion"] = (
            "amount_shock_5_20_raw appears compositionally promising relative to the frozen v18 core family: "
        ...
        judgement["implication_for_next_step"] = (
            "A future family round may test amount_shock_5_20_raw as a constrained third signal under a single-dimension preregistration."
        )
    else:
        judgement["classification"] = "composability_screen_mixed"
        judgement["primary_conclusion"] = (
            "amount_shock_5_20_raw is a real standalone keeper, but its additive fit with the frozen v18 core family is mixed rather than clean. "
        ...
        judgement["implication_for_next_step"] = (
            "Do not promote amount_shock_5_20_raw into a direct equal-weight family replacement yet. If used later, prefer a constrained third-signal test or gated modifier design."
        )

    if momentum_amount_corr["corr_avg_daily"] is not None:
        judgement["mechanism"].append(
            "Pairwise momentum-vs-amount-shock rank correlation is "
            + format_num(momentum_amount_corr["corr_avg_daily"])
            + " on average by day, which indicates the new signal is not simply duplicating momentum."
        )
    if core_amount_corr["corr_avg_daily"] is not None:
        judgement["mechanism"].append(
            "Core-family-vs-amount-shock rank correlation is "
            + format_num(core_amount_corr["corr_avg_daily"])
            + " on average by day, so the candidate contributes partially orthogonal ordering information."
        )
    if bucket_stats.get("core_high_amount_high") is not None and bucket_stats.get("core_high_amount_mid") is not None:
        judgement["mechanism"].append(
            "In the high-core region, core_high+amount_high average label is "
            + format_num(bucket_stats["core_high_amount_high"])
            + " versus core_high+amount_mid "
            + format_num(bucket_stats["core_high_amount_mid"])
    ...
    report = {
        ...
        "research_round_id": ROUND_ID,
        "as_of_date": "20260424",
        "diagnosis_question": "Relative to the frozen v18 core two-signal family, does amount_shock_5_20_raw look compositionally compatible enough to justify a later family-level promotion?",
        ...
        "pairwise_component_correlation": {
            "momentum_vs_amount_shock": momentum_amount_corr,
            "liquidity_trend_vs_amount_shock": liquidity_amount_corr,
            "core_v18_vs_amount_shock": core_amount_corr
        },
    ...
    md_lines = [
        "# amount_shock_5_20 composability screening diagnosis",
        "",
        ...
        "",
        f"- `momentum_vs_amount_shock_corr_avg_daily(动量与量能冲击日均相关) = {format_num(momentum_amount_corr['corr_avg_daily'])}`",
        f"- `liquidity_trend_vs_amount_shock_corr_avg_daily(流动性趋势与量能冲击日均相关) = {format_num(liquidity_amount_corr['corr_avg_daily'])}`",
        f"- `core_v18_vs_amount_shock_corr_avg_daily(v18核心分数与量能冲击日均相关) = {format_num(core_amount_corr['corr_avg_daily'])}`",
        f"- `core_high_amount_high_avg_label(核心高+量能冲击高平均标签) = {format_num(bucket_stats.get('core_high_amount_high'))}`",
        f"- `core_high_amount_mid_avg_label(核心高+量能冲击中平均标签) = {format_num(bucket_stats.get('core_high_amount_mid'))}`",
        f"- `core_high_amount_low_avg_label(核心高+量能冲击低平均标签) = {format_num(bucket_stats.get('core_high_amount_low'))}`",
        "",
