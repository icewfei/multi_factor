...
ROUND_ID = "rr_price_volume_high_open_hold_ratio_composability_screen_20260428"
CANDIDATE_ID = "price_volume_screen_high_open_hold_ratio_20d_composability_v1"
...
RUN_ID = "screendiag_price_volume_high_open_hold_ratio_composability_20260428"
...
def main() -> None:
    ...
    single_signal_report = load_json(
        RESEARCH_DIR
        / "research_rounds"
        / "rr_price_volume_single_signal_discovery_v18_round8_orthogonal_20260428"
        / "price_volume_single_signal_high_open_hold_ratio_20d_v1_signal_edge_diagnosis_20260428.json"
    )

    round_dir = RESEARCH_DIR / "research_rounds" / ROUND_ID
    json_report_path = round_dir / "price_volume_screen_high_open_hold_ratio_20d_composability_v1_diagnosis_20260428.json"
    md_report_path = round_dir / "price_volume_screen_high_open_hold_ratio_20d_composability_v1_diagnosis_20260428.md"
    ...
    try:
        ...
        con.execute(
            f"""
            CREATE OR REPLACE VIEW high_open_hold_ratio_features AS
            WITH bars AS (
                SELECT
                ...
                SELECT
                    ...
                    CASE
                        WHEN LAG(adj_close, 1) OVER w > 0
                             ...
                             AND (adj_open - LAG(adj_close, 1) OVER w) > 0
                        THEN (adj_close - LAG(adj_close, 1) OVER w) / (adj_open - LAG(adj_close, 1) OVER w)
                        ELSE NULL
                    END AS hold_quality_part
                FROM bars
            ...
            SELECT
                instrument,
                signal_date,
                AVG(hold_quality_part) OVER w20 AS high_open_hold_ratio_20d_raw
            FROM daily
            WINDOW
                w20 AS (
        ...
        con.execute(
            ...
            SELECT
                ...
                r.liquidity_rank,
                s.high_open_hold_ratio_20d_raw
            ...
            LEFT JOIN high_open_hold_ratio_features s
        ...
        con.execute(
            ...
            SELECT
                ...
                PERCENT_RANK() OVER (
                    ...
                    ORDER BY high_open_hold_ratio_20d_raw DESC, instrument ASC
                ) AS high_open_hold_ratio_rank
            FROM base_frame
            WHERE ranking_eligible_D0
              AND high_open_hold_ratio_20d_raw IS NOT NULL
            """
        )
        con.execute(
            """
            ...
            SELECT
                ...
                CASE
                    WHEN b.momentum_rank IS NOT NULL
                     ...
                     AND s.high_open_hold_ratio_rank IS NOT NULL
                    THEN (b.momentum_rank + b.liquidity_trend_rank + s.high_open_hold_ratio_rank) / 3.0
                    ELSE NULL
            ...
            rows = con.execute(
                ...
                bucketed AS (
                    SELECT
                        ...
                        NTILE(3) OVER (
                            PARTITION BY signal_date
                            ORDER BY high_open_hold_ratio_rank DESC, instrument ASC
                        ) AS signal_bucket
                    FROM usable
                )
                SELECT
        ...
        momentum_signal_corr = pair_corr("momentum_rank", "high_open_hold_ratio_rank")
        liquidity_signal_corr = pair_corr("liquidity_trend_rank", "high_open_hold_ratio_rank")
        core_signal_corr = pair_corr("core_score_v18", "high_open_hold_ratio_rank")
    ...
    audit = {
        ...
        "notes": [
            ...
            "The screen score is a tentative static three-component average over momentum_60_5_raw, liquidity_trend_20_60_raw, and high_open_hold_ratio_20d_raw.",
    ...
    if top_slice_improved and head_stability_not_worse and liquidity_not_worse and additive_high_bucket:
        ...
        judgement["primary_conclusion"] = (
            "high_open_hold_ratio_20d_raw appears compositionally promising relative to the frozen v18 core family: "
        ...
        judgement["implication_for_next_step"] = (
            "A future family round may test high_open_hold_ratio_20d_raw as a constrained third signal under a single-dimension preregistration."
        )
    else:
        judgement["classification"] = "composability_screen_mixed"
        judgement["primary_conclusion"] = (
            "high_open_hold_ratio_20d_raw is a real standalone keeper, but its additive fit with the frozen v18 core family is mixed rather than clean. "
        ...
        judgement["implication_for_next_step"] = (
            "Do not promote high_open_hold_ratio_20d_raw into a direct equal-weight family replacement yet. If used later, prefer a constrained third-signal test or gated modifier design."
        )

    if momentum_signal_corr["corr_avg_daily"] is not None:
        judgement["mechanism"].append(
            "Pairwise momentum-vs-intraday-reversal-asymmetry rank correlation is "
    ...
    if core_signal_corr["corr_avg_daily"] is not None:
        judgement["mechanism"].append(
            "Core-family-vs-intraday-reversal-asymmetry rank correlation is "
    ...
    report = {
        ...
        "research_round_id": ROUND_ID,
        "as_of_date": "20260428",
        "diagnosis_question": "Relative to the frozen v18 core two-signal family, does high_open_hold_ratio_20d_raw look compositionally compatible enough to justify a later family-level promotion?",
        ...
        "pairwise_component_correlation": {
            "momentum_vs_high_open_hold_ratio": momentum_signal_corr,
            "liquidity_trend_vs_high_open_hold_ratio": liquidity_signal_corr,
            "core_v18_vs_high_open_hold_ratio": core_signal_corr
        },
    ...
    md_lines = [
        "# high_open_hold_ratio_20d composability screening diagnosis",
        "",
        ...
        "",
        f"- `momentum_vs_high_open_hold_ratio_corr_avg_daily(动量与日内反转不对称日均相关) = {format_num(momentum_signal_corr['corr_avg_daily'])}`",
        f"- `liquidity_trend_vs_high_open_hold_ratio_corr_avg_daily(流动性趋势与日内反转不对称日均相关) = {format_num(liquidity_signal_corr['corr_avg_daily'])}`",
        f"- `core_v18_vs_high_open_hold_ratio_corr_avg_daily(v18核心分数与日内反转不对称日均相关) = {format_num(core_signal_corr['corr_avg_daily'])}`",
