...
ROUND_ID = "rr_price_volume_intraday_reversal_asymmetry_composability_screen_20260428"
CANDIDATE_ID = "price_volume_screen_intraday_reversal_asymmetry_20d_composability_v1"
...
RUN_ID = "screendiag_price_volume_intraday_reversal_asymmetry_composability_20260428"
...
def main() -> None:
    ...
    single_signal_report = load_json(
        ...
        / "price_volume_single_signal_intraday_reversal_asymmetry_20d_v1_signal_edge_diagnosis_20260428.json"
    )

    round_dir = RESEARCH_DIR / "research_rounds" / ROUND_ID
    json_report_path = round_dir / "price_volume_screen_intraday_reversal_asymmetry_20d_composability_v1_diagnosis_20260428.json"
    md_report_path = round_dir / "price_volume_screen_intraday_reversal_asymmetry_20d_composability_v1_diagnosis_20260428.md"
    ...
    try:
        ...
        con.execute(
            f"""
            CREATE OR REPLACE VIEW intraday_reversal_asymmetry_features AS
            WITH bars AS (
                SELECT
            ...
            daily AS (
                SELECT
                    ...
                    signal_date,
                    CASE
                        WHEN adj_high > adj_low AND adj_open > 0 AND adj_close / adj_open - 1.0 < 0
                        THEN (adj_close - adj_low) / (adj_high - adj_low)
                        ELSE NULL
                    END AS down_recovery_part,
                    CASE
                        WHEN adj_high > adj_low AND adj_open > 0 AND adj_close / adj_open - 1.0 > 0
            ...
            SELECT
                instrument,
                signal_date,
                AVG(down_recovery_part) OVER w20 - AVG(up_fade_part) OVER w20 AS intraday_reversal_asymmetry_20d_raw
            FROM daily
            WINDOW
                w20 AS (
        ...
        con.execute(
            ...
            SELECT
                ...
                r.liquidity_rank,
                s.intraday_reversal_asymmetry_20d_raw
            ...
            LEFT JOIN intraday_reversal_asymmetry_features s
            ...
            SELECT
                ...
                CASE
                    WHEN b.momentum_rank IS NOT NULL
                     ...
                     AND s.intraday_reversal_asymmetry_rank IS NOT NULL
                    THEN (b.momentum_rank + b.liquidity_trend_rank + s.intraday_reversal_asymmetry_rank) / 3.0
                    ELSE NULL
                ...
                SELECT
                    ...
                    CASE
                        WHEN momentum_rank IS NOT NULL
                         ...
                         AND intraday_reversal_asymmetry_rank IS NOT NULL
                        THEN 3
                        ELSE (CASE WHEN momentum_rank IS NOT NULL THEN 1 ELSE 0 END)
                           + (CASE WHEN liquidity_trend_rank IS NOT NULL THEN 1 ELSE 0 END)
                           + (CASE WHEN intraday_reversal_asymmetry_rank IS NOT NULL THEN 1 ELSE 0 END)
            ...
            rows = con.execute(
                ...
                bucketed AS (
                    SELECT
                        ...
                        NTILE(3) OVER (
                            PARTITION BY signal_date
                            ORDER BY intraday_reversal_asymmetry_rank DESC, instrument ASC
                        ) AS signal_bucket
                    FROM usable
                )
                SELECT
        ...
        momentum_signal_corr = pair_corr("momentum_rank", "intraday_reversal_asymmetry_rank")
        liquidity_signal_corr = pair_corr("liquidity_trend_rank", "intraday_reversal_asymmetry_rank")
        core_signal_corr = pair_corr("core_score_v18", "intraday_reversal_asymmetry_rank")
    ...
    audit = {
        ...
        "notes": [
            ...
            "The screen score is a tentative static three-component average over momentum_60_5_raw, liquidity_trend_20_60_raw, and intraday_reversal_asymmetry_20d_raw.",
    ...
    if top_slice_improved and head_stability_not_worse and liquidity_not_worse and additive_high_bucket:
        ...
        judgement["primary_conclusion"] = (
            "intraday_reversal_asymmetry_20d_raw appears compositionally promising relative to the frozen v18 core family: "
        ...
        judgement["implication_for_next_step"] = (
            "A future family round may test intraday_reversal_asymmetry_20d_raw as a constrained third signal under a single-dimension preregistration."
        )
    else:
        judgement["classification"] = "composability_screen_mixed"
        judgement["primary_conclusion"] = (
            "intraday_reversal_asymmetry_20d_raw is a real standalone keeper, but its additive fit with the frozen v18 core family is mixed rather than clean. "
        ...
        judgement["implication_for_next_step"] = (
            "Do not promote intraday_reversal_asymmetry_20d_raw into a direct equal-weight family replacement yet. If used later, prefer a constrained third-signal test or gated modifier design."
        )
    ...
    report = {
        ...
        "research_round_id": ROUND_ID,
        "as_of_date": "20260428",
        "diagnosis_question": "Relative to the frozen v18 core two-signal family, does intraday_reversal_asymmetry_20d_raw look compositionally compatible enough to justify a later family-level promotion?",
        ...
        "pairwise_component_correlation": {
            "momentum_vs_intraday_reversal_asymmetry": momentum_signal_corr,
            "liquidity_trend_vs_intraday_reversal_asymmetry": liquidity_signal_corr,
            "core_v18_vs_intraday_reversal_asymmetry": core_signal_corr
        },
    ...
    md_lines = [
        ...
        "",
        f"- `momentum_vs_intraday_reversal_asymmetry_corr_avg_daily(动量与日内反转不对称日均相关) = {format_num(momentum_signal_corr['corr_avg_daily'])}`",
        f"- `liquidity_trend_vs_intraday_reversal_asymmetry_corr_avg_daily(流动性趋势与日内反转不对称日均相关) = {format_num(liquidity_signal_corr['corr_avg_daily'])}`",
        f"- `core_v18_vs_intraday_reversal_asymmetry_corr_avg_daily(v18核心分数与日内反转不对称日均相关) = {format_num(core_signal_corr['corr_avg_daily'])}`",
