...
SCRIPT_PATH = Path("scripts/diagnose_baseline_overlap_divergence.py")


def load_module():
    spec = spec_from_file_location("diagnose_baseline_overlap_divergence", SCRIPT_PATH)
            ...
            for instrument_idx, instrument in enumerate(instruments):
                rows.append(
                    (
                        ...
                        100.0 + instrument_idx * 25.0 + idx,
                        1000.0 + instrument_idx * 150.0 + idx * 10.0,
                        800.0 + instrument_idx * 100.0 + idx * 8.0,
...
def build_fixture(tmp_path: Path) -> dict[str, Path]:
    ...
    for idx, trade_date in enumerate(trade_dates):
        ...
        confirmed5_order = ["AAA.SZ", "CCC.SZ", "DDD.SZ", "BBB.SZ"] if idx % 2 == 0 else ["BBB.SZ", "DDD.SZ", "CCC.SZ", "AAA.SZ"]
        v2_order = ["AAA.SZ", "CCC.SZ", "BBB.SZ", "DDD.SZ"] if idx % 3 else ["DDD.SZ", "CCC.SZ", "AAA.SZ", "BBB.SZ"]
...
def test_compute_pairwise_overlap() -> None:
    ...
    frame = pd.DataFrame(
        [
            {"split_bucket": "train", "signal_date": "20200101", "cohort": "overlap", "instrument": "A"},
            {"split_bucket": "train", "signal_date": "20200101", "cohort": "baseline_only", "instrument": "B"},
            {"split_bucket": "train", "signal_date": "20200101", "cohort": "nonlinear_only", "instrument": "C"},
            {"split_bucket": "validation", "signal_date": "20200102", "cohort": "overlap", "instrument": "A"},
            {"split_bucket": "validation", "signal_date": "20200102", "cohort": "overlap", "instrument": "B"},
            {"split_bucket": "validation", "signal_date": "20200102", "cohort": "baseline_only", "instrument": "C"},
            {"split_bucket": "validation", "signal_date": "20200102", "cohort": "nonlinear_only", "instrument": "D"},
        ]
    )
    result = module.compute_pairwise_overlap(frame, topk=2)
    assert result["train"]["avg_overlap_count"] == 1.0
    assert round(result["train"]["avg_jaccard"], 4) == 0.3333
    assert result["validation"]["avg_overlap_count"] == 2.0
...
def test_script_runs_on_small_fixture(repo_root: Path, tmp_path: Path) -> None:
    ...
    output_json = tmp_path / "baseline_overlap_divergence.json"
    output_md = tmp_path / "baseline_overlap_divergence.md"
    ...
    assert "不建议进入 divergence-aware challenger" in markdown
